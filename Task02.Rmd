...
4. Small proportion of outliers across features tells us that Winsorisation is not necessary. We will use a trimmed mean imputation instead to reduce the influence of extreme values.
...
```
The preprocessing recipe removes all non-informative Info_ columns from the predictor set, imputes missing values using trimmed mean (10% trim) and normalizes all numeric features. The only unaddressed insight from EDA is the presence of highly correlated features, which we will address during modelling using PCA. 
...
:::
Class imbalance obtained from EDA is moderate, therefore no strategy was employed.
```{r modelling_step1}
...
```
We have defined a workflow that involves tuning 3 hyperparameters: mtry, min_n and num_comp. We also ensure we use grouped CV folds to prevent data leakage. The best configuration achieved an F1 score of 0.787 (std_error = 0.0215) with mtry = 5, min_n = 6 and num_comp = 80.

The best performing configurations consistently used the largest value of num_comp (80) in the grid, suggesting that increasing num_comp may marginally improve the F1 score. However, as 80 is already a substantial number of principal components, further increases would likely give diminishing returns while significantly increasing computational cost. Therefore, we proceed with the obtained configuration.
...
test_metrics
```
The Factor levels for Class remain in their original order (-1, 1). As a result, yardstick treats -1 as the positive class while computing the F1 score. The test metrics tell us that our model achieves an F1 score of 0.767 on the test set, close to the cross-validated estimate of 0.787. This tells us that the model generalizes well.
