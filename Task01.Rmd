...
```
Q-Q plots show residuals are approximately normally distributed with minor tail deviations. Residuals vs Fitted and Scale-Location plot indicates relatively constant variance across groups. Violin plot shows similar spread across methods.
...
:::{#Q1_1_a .message style="color: black; border: 1px outset black; background-color: #eeeffe;"}
We now test whether the candidate DE configurations have any statistically significant difference in the expected mean loss across the class of test problems of interest. We will perform a two-sided omnibus test at alpha = 0.04, running each method on the same problems. If the omnibus test is significant we will run all-vs-one Dunnett comparisons of the means of the methods.
...
alpha <- exp_pars$alpha
mres <- exp_pars$mres
power <- exp_pars$desired.power
K <- length(method.names)
n_comparisons <- K - 1

alpha_bon <- alpha / n_comparisons
z_alpha <- qnorm(1 - alpha_bon / 2)
z_beta <- qnorm(power)

nprobs <- ceiling((z_alpha + z_beta) ^ 2 / (mres ^ 2))
nprobs
...
for (p in seq_along(problem.names)) {
  for (m in method.names) {
    for (r in seq_len(exp_pars$nruns)) {
      x <- ExpDE2(method.name = m, problem.name = problem.names[p])
      
      results_list[[idx]] <- data.frame(
        Method = m,
        Problem = problem.names[p],
        Problem_type = problem.types[p],
        Problem_dimension = problem.dimensions[p],
        Value = log(as.numeric(x))
      )
      idx <- idx + 1
    }
  }
...
myres2 <- do.call(rbind, results_list)

myres2$Method <- factor(myres2$Method)
myres2$Problem <- factor(myres2$Problem)
myres2$Problem_type <- factor(myres2$Problem_type)

myres2$Method <- relevel(myres2$Method, ref = best_method)
...
```{r Q1_2_d_code}
# ADD YOUR CODE HERE
mytest02 <- aov(Value ~ Method + Problem, data = myres2)
summary(mytest02)
...
The ANOVA test with Problem as a blocking factor indicates a statistically significant difference in the expected mean log-loss across the five candidate methods. The p-value obtained is extremely small and much less than the predefined significance level of alpha = 0.04. Therefore, we reject the null hypothesis and conclude that at least one method's expected mean performance differs significantly from the others when generalizing over the class of test problems.
...
# ADD YOUR CODE HERE
ci_level <- 1 - exp_pars$alpha
myCIs03 <- confint(myMHT2, level = ci_level)
print(myCIs03)
...
Based on the results of our generalized experiment across the class of test problems, the recommended strategy is DE.2704500.E. This method has the smallest estimated mean log-loss across all problems and repeated runs and is therefore the best performing method overall.

Compared with the results of the preliminary test, this is not the same method we identified previously (DE.2704500.A). While A was the best method for the single problem type we tested on, this does not hold true when the scope of problems is broadened. The Dunnett CIs for the comparison between E and A does not include 0, telling us that E is statistically better than A at the alpha = 0.04 level.

If we had to choose a second best algorithm, our results would point us to DE.2704500.D, which had the next lowest mean loss after E. The Dunnett comparision between D and A also does not include zero, confirming D is also significantly better than A. However, the difference between D and E is substantial.

Overall, DE.2704500.E is the method that produced the lowest estimated log-loss across all problems and is therefore the recommended strategy.
